...
- [Organization](#organization)
  ...
  - [Adequate processes](#adequate-processes)
...
- [Development](#development)
  ...
  - [Version control and reviews](#version-control-and-reviews)
...
software components, running with minimal human intervention.

Sections of the document are ordered by decreasing level of abstraction, feel
free to skip those that seem to be non-substantial.

The markdown source of this document is available at
<https://github.com/asherikov/codeincomplete>, an HTML version – at
<http://www.sherikov.net/codeincomplete/>.
...
It feels that many teams fail at adequately matching the complexity of the
processes with the team size and project scale. 5-people teams spend a third of
their time writing KPIs and doing sprint reviews, while 50-people teams do not
bother to track their tasks in Jira. Both extremes negatively impact
performance, so it is crucial to find a reasonable balance and evolve processes
to reflect changes in the team and the projects.

Communication
-------------

Work related communication should happen through channels that facilitate
tracing and sharing of information and decisions: wiki, email, issue tracker,
source code management systems, etc. It is a crime to ignore your notifications
and expect everyone to reach you in-person to resolve issues.
...
### Separate representation and handling

Separate data containers from logic that uses or modifies the data, e.g., use
separate classes for a message from a message publisher.
...
- Minimize size of merge requests to facilitate reviews.
...
- Keep in mind that asserts are disabled for C++ when using the default
  `RelWithDebInfo` build type. So you may want to use a different build type
  when testing code that uses many asserts, e.g., `Eigen` would produce much
...
Gravity is not constant: it depends on position on the globe and altitude – in
some applications this variation is non-negligible.
...
- 24 hour, aka ‘military’, time format is easier to read and parse and should
  always be preferred to a.m./p.m. convention.

- Use integers instead of floats for storing timestamps: time is inherently
  discrete in computer systems, and, more importantly, precision of floating
  point numbers varies depending on the magnitude. Floating point timestamp
  accuracy is going to gradually decrease in the future, leading to
  deterioration of performance in time-critical applications, e.g., state
  estimators, etc.
...
- Do not impose 80-char line length constraint: it is a legacy terminal
  restriction that makes no sense on modern screens, use a larger limit if
  appropriate.
...
- Enforce language standard using corresponding compiler flag. Don’t use any
  language extensions: `GNU`, etc.
...
- Do not use names starting with underscore: such names are implicitly reserved
  by system libraries.
...
- Member declaration order defines the destruction order, bad ordering may lead
  to memory access failures:
...
- Often it is recommended to declare members as `private` instead of
...
- Do not perform automatic inclusion sorting using a formatting tool, it tends
  ...
  behave inconsistently when using `clang-format`, see
  <https://github.com/llvm/llvm-project/issues/81118>.
...
- Use explicit template instantiations when possible
  <https://www.en.cppreference.com/w/cpp/language/class_template.html>.

- Static libraries that are reused multiple times in a project increase
  compilation time and the total size of the project binaries. Shared libraries
  should be used by default.

### Patterns

- Singleton pattern should only be used when absolutely necessary: in most cases
  it is an unnecessary limitation. There is absolutely no need to limit your
  application to a single log file, etc.
...
- ROS1 catkin package template example
  <https://github.com/asherikov/ccws/tree/master/ccws/examples/pkg_catkin>, ROS2
  ament package example
  <https://github.com/asherikov/ccws/tree/master/ccws/examples/pkg_ament>
...
- It is a good practice to put several related packages in the same repository
  to simplify maintenance: there is less repositories to work with, related
  packages are often need to be modified simultaneously, etc.
...
- Don’t put your headers in the root of system `**/include/` folders, it is like
  peeing in public. Always use package specific subdirectories.
